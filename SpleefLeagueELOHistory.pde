...
void loadData() {
  ...
  String[] tournaments_txt = loadStrings("/data/tournaments.txt");
  Table table;
  for (String s : tournaments_txt) {
    table = loadTable("/data/tournaments/"+s+".csv");
...
for (Tournament t : tournaments) {
  ...
  for (int i = 0; i < ta.getRowCount();i++) {
    ...
    String country2 = data.checkCountry(perdedor);
       ...
       if (!(names.contains(ganador))) {
         names.add(ganador);
        Player p1 = new Player(ganador,country1);
         players.add(p1);
       }
    if (!(names.contains(perdedor))) {
         names.add(perdedor);
 ...
 unknown = loadImage("/data/countries/Unknown.png");
  ...
  int calculateELO(Player winner, Player loser) {
    int k = 48;
...
public void doBackground() {
  ...
  image(loadImage("third_place.png"),825,123,24,34);
    ...
    data.finalStats();
      ...
      Iterator iterator = topWins.entrySet().iterator();
     int as = 0;
      while (iterator.hasNext() && as==0) {
          Map.Entry<String,Integer> pair = (Map.Entry<String,Integer>)iterator.next();
         String player = pair.getKey();
         Integer value = pair.getValue();
          text("most wins:"+ player + " " + value,200,200);
           as = 1;
      }
